...
# process 1
Observable.from_(greek_words).map(lambda s: intense_calc(s)).subscribe_on(
    pool_scheduler
).subscribe(
...
# process 2
Observable.range(1, 10).map(lambda s: intense_calc(s)).subscribe_on(
    pool_scheduler
).subscribe(
...
Observable.interval(1000).map(lambda i: i * 100).observe_on(pool_scheduler).map(
    lambda s: intense_calc(s)
).subscribe(
