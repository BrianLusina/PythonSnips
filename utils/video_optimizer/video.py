...
FFMPEG_OPTIONS = (
    "-codec:v libx264 -profile:v main -preset slow -movflags faststart -b:v 300k -maxrate 300k "
...
def optimize(source, force_yes, workers):
    ...
    matches = []
    for filename in iter_matching(
        source, re.compile(r".*\.(mp4|wmv|ogv|avi|flv)$").match
    ):
...
def ffmpeg(file_input, root_output="output"):
    ...
    output_dir = os.path.join(
        root_output,
        os.path.normpath(os.path.relpath(os.path.dirname(file_input), source_dir)),
    )
    ...
    try:
        ff = ffmpy.FF(
            inputs={file_input: None},
            outputs={
                os.path.abspath(
                    os.path.join(output_dir, os.path.basename(file_input))
                ): FFMPEG_OPTIONS
            },
        )
    ...
    return (
        ...
        + str(duration)
        + " in ~"
        + str(round((end_time - start_time) / 1000 / 60))
    )
