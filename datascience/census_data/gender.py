...
x_train, x_test, y_train, y_test = train_test_split(
    fin_data.values, y, test_size=0.2, random_state=1
)
...
for i in range(0, 14):
    if i == 1:
        ...
        attr = input(
            col_names[i]
            + "(Bachelors, HS-grad, Masters, 7th-8th, 9th, 11th, Some-college, Assoc-acdm, Assoc-voc): "
        )
    elif i == 5:
        attr = input(
            col_names[i]
        ...
        attr = input(
            col_names[i]
            + "(Adm-clerical, Exec-manager, Handlers-cleaners, Exec-managerial, Prof-specialty, Sales, Protective-serv, Craft-repair, Transport-moving, Farming-fishing, Machine-op-inspct, Tech-support, Other-service): "
        )
    elif i == 7:
        attr = input(
            col_names[i] + "(Not-in-family, Husband, Wife, Local-gov, Federal-gov): "
        )
    elif i == 8:
        attr = input(
            col_names[i] + "(White, Black, Asian-Pac-Islander, Amer-Indian-Eskimo): "
        )
    elif i == 9:
