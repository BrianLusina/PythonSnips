    ...
    def test_it_does_not_accept_integers(self):
        with self.assertRaises(ValueError) as context:
            ...
            self.assertEqual(
                ...
                "Invalid input of type int not allowed",
            )

    def test_calculated_tax_is_a_float(self):
        result = calculate_tax({"Jane": 20500})
        self.assertIsInstance(
            calculate_tax({"Jane": 20500}),
            dict,
            msg="Should return a result of data type dict",
        )
        self.assertIsInstance(
            result["Jane"], float, msg="Tax returned should be an float."
        )

    def test_it_returns_zero_tax_for_income_less_than_1000(self):
        result = calculate_tax({"Jake": 100})
        self.assertEqual(
            result, {"Jake": 0}, msg="Should return zero tax for incomes less than 1000"
        )
