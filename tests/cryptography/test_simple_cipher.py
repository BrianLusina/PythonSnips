...
class CipherTest(unittest.TestCase):
    ...
    def test_cipher_encode1(self):
        c = Cipher("a")
        self.assertEqual(
            "itisawesomeprogramminginpython", c.encode("itisawesomeprogramminginpython")
        )
    ...
    def test_cipher_encode4(self):
        key = (
            "duxrceqyaimciuucnelkeoxjhdyduucpmrxmaivacmybmsdrzwqxvbxsy"
    ...
    def test_cipher_random_key(self):
        c = Cipher()
        self.assertTrue(
            len(c.key) >= 100, "A random key must be generated when no key is given!"
        )
        self.assertTrue(
            c.key.islower() and c.key.isalpha(),
