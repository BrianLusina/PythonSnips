...
from cryptography.rail_fence_cipher import decode, encode


class RailFenceTests(unittest.TestCase):
    def test_encode_with_two_rails(self):
        self.assertMultiLineEqual("XXXXXXXXXOOOOOOOOO", encode("XOXOXOXOXOXOXOXOXO", 2))

    def test_encode_with_three_rails(self):
        self.assertMultiLineEqual(
            "WECRLTEERDSOEEFEAOCAIVDEN", encode("WEAREDISCOVEREDFLEEATONCE", 3)
        )
    ...
    def test_decode_with_three_rails(self):
        self.assertMultiLineEqual(
            "THEDEVILISINTHEDETAILS", decode("TEITELHDVLSNHDTISEIIEA", 3)
        )

    def test_decode_with_five_rails(self):
        self.assertMultiLineEqual("EXERCISMISAWESOME", decode("EIEXMSMESAORIWSCE", 5))
