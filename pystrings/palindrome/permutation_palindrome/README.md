...
understanding of the problem doesn't look good in an interview. We can do this in O(n) time.

A brute force algorithm simply enumerates all possible answers to the question and checks
...
What would be the time cost? For a string of length n, there are n! permutations (n choices for the first character,
times n−1 choices for the second character, etc). Checking each length-n permutation to see if it's a palindrome
involves checking each character, taking O(n) time. That gives us O(n!n) time overall. We can do better!
...
What if we just track whether each character appears an odd number of times? Then we can map characters to
