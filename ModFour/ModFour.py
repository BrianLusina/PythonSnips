...
Test.test_function(type(ModFour.mod) == type(re.compile("")), True)
...
for test in validTests:
    Test.test_function(ModFour.mod(test) is not None, True)
...
for test in invalidTests:
    Test.test_function(ModFour.mod(test) is not None, False)

r = re.compile('^\[\W?(?:[048]|[0-9]*(?:[02468][048]|[13579][26]))\]$')
rS = r.search("[+002016]")
print rS.group()
"""
The digits 0, 4 and 8 only follow even numbers while the digits 2 and 6 always follow odd numbers:
