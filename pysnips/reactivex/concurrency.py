...
# process 1
Observable.from_(greek_words).map(lambda s: intense_calc(s)).subscribe_on(pool_scheduler).subscribe(on_next=lambda s: print("PROCESS 1: {0} {1}".format(current_thread().name, s)),
               on_error=lambda e: print(e),
               on_completed=lambda: print("PROCESS 1 done!"))

# process 2
Observable.range(1, 10).map(lambda s: intense_calc(s)).subscribe_on(pool_scheduler).subscribe(on_next=lambda i: print("PROCESS 2: {0} {1}".format(current_thread().name, i)),
