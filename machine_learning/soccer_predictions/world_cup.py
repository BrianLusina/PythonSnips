...
def validate(label, target, predictions, baseline=0.5, compute_auc=False, quiet=True):
    ...
    if len(target) != len(predictions):
        raise Exception("Length mismatch %d vs %d" % (len(target), len(predictions)))
    if baseline > 1.0:
    ...
    if compute_auc:
        ...
        pl.plot(fpr, tpr, lw=1.5, label="ROC %s (area = %0.2f)" % (label, auc_value))
    ...
    if not quiet:
        print(
            "    Base: %0.03g Acc: %0.03g P(1|t): %0.03g P(0|f): %0.03g"
            % (baseline, accuracy, prob1_t, prob0_f)
        )
        print(
            "    Fp/Fn/Tp/Tn p/n/c: %d/%d/%d/%d %d/%d/%d"
...
def extract_predictions(data, predictions):
    ...
    for game in range(len(data) / 2):
        if data["matchid"].iloc[game * 2] != data["matchid"].iloc[game * 2 + 1]:
            raise Exception(
                "Unexpeted match id %d vs %d",
                (data["matchid"].iloc[game * 2], data["matchid"].iloc[game * 2 + 1]),
            )
...
def _check_data(data):
    ...
    while i < len(data) - 1:
        if matches.iloc[i] != matches.iloc[i + 1]:
            raise Exception(
                "Match mismatch: %s vs %s " % (matches.iloc[i], matches.iloc[i + 1])
            )
        if teams.iloc[i] != op_teams.iloc[i + 1]:
            raise Exception(
                "Team mismatch: match %s team %s vs %s"
                % (matches.iloc[i], teams.iloc[i], op_teams.iloc[i + 1])
            )
        if teams.iloc[i + 1] != op_teams.iloc[i]:
            raise Exception(
                "Team mismatch: match %s team %s vs %s"
                % (matches.iloc[i], teams.iloc[i + 1], op_teams.iloc[i])
            )
        i += 2
