...
def _build_team_matrix(data, target_col):
    ...
    for game in range(nrows):
        ...
        teams[home_id][game] = (1.0 + home["is_home"] * 0.25) / current_discount
        teams[away_id][game] = (-1.0 - away["is_home"] * 0.25) / current_discount
        result[game] = points
...
def _build_power(games, outcomes, coerce_fn, acc=0.0001, alpha=1.0, snap=True):
    ...
    model = build_model_logistic(outcomes, games, acc=acc, alpha=alpha)
...
def _get_power_map(competition, competition_data, col, coerce_fn):
    ...
    while True:
        if alpha < 0.1:
            print(
                "Skipping power ranking for competition %s column %s"
                % (competition, col)
            )
            return {}
        try:
            ...
            outcomes = games[col]
            del games[col]
            competition_power = _build_power(
                games, outcomes, coerce_fn, acc, alpha, snap=False
            )
            if not competition_power:
                alpha /= 2
                print(
                    "Reducing alpha for %s to %f due lack of range"
                    % (competition, alpha)
                )
            else:
            ...
            alpha /= 2
            print(
                "Reducing alpha for %s to %f due to error %s"
...
def add_power(data, power_train_data, cols):
    ...
    for (col, coerce_fn, final_name) in cols:
        ...
        for competition in competitions:
            ...
            power.update(_get_power_map(competition, competition_data, col, coerce_fn))
