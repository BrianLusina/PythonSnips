...
class MessageRevokedHandler(EventHandler):
    """Handles MESSAGE_REVOKED events"""

    def can_handle(self, event: Event) -> bool:
        return event.event_type == EventType.MESSAGE_REVOKED

    def get_target_state(self, event: Event) -> Optional[MessageState]:
        return MessageState.REVOKED
    ...
    def handle(self, event: Event, message: Message) -> bool:
        ...
        if StateTransitionConfig.is_valid_transition(
            ...
            logging.info(f"Message {message.message_id} revoked")
            return True
        return False
